...
| Auth (hosts) | **Better Auth** on Workers + D1 with email magic links sent via **MailChannels** (free with Cloudflare Workers) | No third-party auth vendor; magic link UX matches Notion/Linear; runs entirely on our stack |
...
1. Host signup + login (magic link via Better Auth + MailChannels)
...
3. **Better Auth** wiring at `src/lib/auth.ts` using the `drizzle` adapter + magic-link plugin. MailChannels for delivery. Add `/api/auth/[...all].ts` route handler.
